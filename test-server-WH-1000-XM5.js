...
class SonySocketServer {
    ...
    async _sendMultipleBattery(output) {
        const devPayload = [PayloadType.POWER_NTFY_STATUS, 1, 39, 1, 40, 1];
        await this._write(output, this._protocol.encodeMessage(MessageType.COMMAND_1,
            devPayload), 'LRBATT');
        const devPayload2 = [PayloadType.POWER_NTFY_STATUS, 2, 38, 0];
        await this._write(output, this._protocol.encodeMessage(MessageType.COMMAND_1,
            devPayload2), 'CASE');
    ...
    async _startTestCycle(output) {
        ...
        for (const step of steps) {
            try {
                ...
                switch (step.type) {
                    case 'ANC': {
                        payload = [PayloadType.NCASM_RET_PARAM, 0x17];
                        if (step.mode === 'OFF')
                            payload.push(0x00, 0x00, 0x00, 0x01, 0x00, 0x00);
                        else if (step.mode === 'ON')
                            payload.push(0x17, 0x00, 0x01, 0x01, 0x00, 0x00);
                        else if (step.mode === 'WIND')
                            payload.push(0x017, 0x02, 0x01, 0x01, 0x00, 0x00);
                        else if (step.mode === 'AMBIENT')
                            payload.push(0x017, 0x00, 0x00, 0x01, 0x01, 0x0A);
                        else if (step.mode === 'AMBIENT15')
                            payload.push(0x017, 0x00, 0x00, 0x01, 0x01, 0x0E);
                        else if (step.mode === 'AMBIENT05')
                            payload.push(0x017, 0x00, 0x00, 0x01, 0x01, 0x05);
                        else if (step.mode === 'AMBIENTFV')
                            payload.push(0x017, 0x00, 0x00, 0x01, 0x00, 0x05);
                    ...
                    case 'S2C': {
                        payload = [PayloadType.SYSTEM_RET_PARAM,
                            0x0C, step.enable ? 0x00 : 0x01, 0x01];
                    ...
                    case 'S2CS': {
                        payload = [PayloadType.SYSTEM_RET_EXT_PARAM,
                            0x0C, step.mode, 0x00];
                    ...
                    case 'S2CT': {
                        payload = [PayloadType.SYSTEM_GET_EXT_PARAM,
                            0x05, 0x00, step.mode];
                    ...
                    case 'VOICE_NOTIFICATION': {
                        payload = [PayloadTypeV2.VOICE_GUIDANCE_RET_PARAM, 0x03,
                            step.enable ? 0x00 : 0x01];
                    ...
                    case 'EQ_PRESET': {
                        payload = [PayloadType.EQEBB_RET_PARAM, 0x01, step.preset,
                    ...
                    case 'EQ_CUSTOM': {
                        payload = [PayloadType.EQEBB_RET_PARAM, 0x01,
                    ...
                    case 'AUDIO_UPSAMPLING': {
                        payload = [PayloadType.AUDIO_RET_PARAM, 0x01,
                    ...
                    case 'PAUSE_WHEN_TAKEN_OFF': {
                        payload = [PayloadType.SYSTEM_RET_PARAM, 0x01,
                            step.enable ? 0x00 : 0x01];
                    ...
                    case 'AUTO_POWER_OFF': {
                        ...
                        if (!cfg)
                            break;
                        payload = [PayloadType.POWER_RET_PARAM, 0x05, ...cfg.bytes];
                        await this._write(output, this._protocol.encodeMessage(
